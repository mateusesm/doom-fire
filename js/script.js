(function init() {
    ...
    const renderFireCanvas = (fireIntensity, arrayWithFire) => {
        ...
        for (let i = 0; i < line; i++) {
            ...
            for (let j = 0; j < column; j++) {
                ...
                if (debug) {
                    table += `<td>${arrayWithFire[i]}<span>${i}</span></td>`
                    index++   
                } else {
                    const color = fireColorsPalette[fireIntensity]
                    const colorString = `${color.r}, ${color.g}, ${color.b}`
            
                    table += `<td style='background-color: rgb(${colorString});'>
                    </td>`
                } 
    ...
    const start = () => {
        ...
        setInterval(() => {
            ...
            renderFireCanvas(fireIntensity, arrayWithFire)
